...
class Form extends \util\Html {
     ...
     *
     * Eg:
     * If all text input fields should be wrapped in `<span />` tags, you can configure the template string mappings per the following:
     *
     * $this->form->config(array('templates' => array(
     * 	'text' => '<span><input type="text" name="{:name}"{:options} /></span>'
     * )));
     *
     * Or, you can re-map one type as another. This is useful if, for example, you
     * include your own helper with custom form template strings which do not match the default template string names.
     *
    ...
    /**
     * The data object to which the current form is bound. In order to
	...
	$defaults = array(
		...
		'id' => function($method, $name, $options) use (&$self) {
			//echo "lambda f, attributes[id]: method=$method, name=$name\n";
	...
	$this->_handlers += array(
	    'wrap' => '_attributes',
	    'url' => function($param, array $options = array()) {
		    if (is_string($param))
			return $param;
	...
	//2015-07-05: add option to set binding
     ...
     * @param mixed $binding the object to bind the form to. This is usually an
	...
	//emulate PUT/DELETE/etc via POST
	...
	return $this->_render($tpl, compact('url', 'options', 'append'));
    }
	...
	$result = $this->_render($template, $options);
	//unset($this->_binding);
	$this->_binding = null;
	return $result;
    }
	...
	if ($name) {
	    return $this->_config['binding']($this->_binding, $name);
	}
	...
	$template = $this->_strings[$template] ? $template : 'input';
	...
	if ($notText && isset($this->_strings['field-' . $type])) {
	    $template = 'field-' . $type;
	}
...
//		$options['label'] = Inflector::humanize(preg_replace('/[\[\]\.]/', '_', $name));
		$options['label'] = preg_replace('/[\[\]\.]/', '_', $name);
	    }
	...
	return $this->_render($template, compact('wrap', 'label', 'input', 'error'));
    }
	...
	return $this->_render('button', $arguments, $scope);
    }
	...
	return $this->_render($template, compact('title', 'options'));
    }
	...
	return $this->_render($template, compact('name', 'options', 'value'));
    }
	...
	list($name, $options, $template) = $this->_defaults(__FUNCTION__, $name, $options);
	return $this->_render($template, compact('name', 'options'));
    }
	...
	foreach ($list as $value => $title) {
		...
		(is_object($scope['value']) && method_exists($scope['value'], '__toString') && ((string) $scope['value'] === (string) $value))
	    );
	    ...
	    $result .= $this->_render('select-option',  $params);
	}
	return $result;
    }
	...
	//echo 'XXX' . print_r($this->binding($key)->data, true) . 'YYY';
	if (!isset($options['checked']) && ($bound = $this->binding($key)->data)) {
	    //SC: 2019.02.10: added option for checkbox array
	    if (is_array($bound))
		$options['checked'] = in_array($default, $bound);
	    else
	...
	return $out . $this->_render($template, compact('name', 'options'));
    }
...
//		list($title, $options) = each($title);
		$__t = $title;
		foreach($__t as $title => $options) break;
	}
	$title = $title ?: str_replace('.', '_', $id);
	...
	return $this->_render($template, compact('id', 'title', 'options'), $scope);
    }
	...
	if (!$content = $this->binding($name)->errors) {
	    return null;
	}
	$result = '';

	if (!is_array($content)) {
	    return $this->_render($template, compact('content', 'options'));
	}
	$errors = $content;

	if ($key === null) {
	    foreach ($errors as $content) {
		$result .= $this->_render($template, compact('content', 'options'));
	    }
	    return $result;
	}
